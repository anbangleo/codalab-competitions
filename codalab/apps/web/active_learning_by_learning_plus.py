...
class ActiveLearningByLearningPlus(QueryStrategy):
    ...
    def __init__(self, *args, **kwargs):
        super(ActiveLearningByLearningPlus, self).__init__(*args, **kwargs)
    ...
    @inherit_docstring_from(QueryStrategy)
    def make_query(self,return_score=True):
        askidlist = []
        ...
        while self.budget_used < self.T:
            ...
            # print self.query_dist   
            # print np.where(self.query_dist==np.max(self.query_dist))
            print ask_idx
            ...
            if ask_id in unlabeled_entry_ids:
                ...
                if return_score:
                    return ask_id,list(zip(unlabeled_entry_ids, self.query_dist))
                else:
                    return ask_id
            else:
                self.update(ask_id, dataset.data[ask_id][1])
